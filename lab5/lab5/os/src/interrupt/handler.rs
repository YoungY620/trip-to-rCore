...
pub fn handle_interrupt(context: &mut Context, scause: Scause, stval: usize) -> *mut Context {
    ...
    // 首先检查线程是否已经结束（内核线程会自己设置标记来结束自己）
    {
        ...
        if current_thread.as_ref().inner().dead {
            println!("hello? in interrupt thread dead.");
...
fn supervisor_timer(context: &mut Context) -> *mut Context{
    println!("hello? in timmer interrupt");
    timer::tick();
    PROCESSOR.lock().park_current_thread(context);
    PROCESSOR.lock().prepare_next_thread()
}

fn page_fault(tf:&mut Context, scause: Scause, stval: usize) -> *mut Context {
    println!("{:?} va = {:#x} instruction = {:#x}", scause.cause(), stval, tf.sepc);
    panic!("page fault!");
}
